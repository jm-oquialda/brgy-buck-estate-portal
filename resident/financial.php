<?php
$pageTitle = 'Financial Assistance Details';
...
$id   = (int)($_GET['id'] ?? 0);
$stmt = $pdo->prepare("SELECT * FROM financial_requests WHERE id=? AND user_id=?");
$stmt->execute([$id, $user['id']]);
$req  = $stmt->fetch();
if (!$req) { setFlash('error', 'Application not found.'); header('Location: /resident/dashboard.php'); exit; }

require_once __DIR__ . '/../includes/header.php';
?>
<div class="page-header">
    <div class="container">
        <h1 class="page-header__title">Financial Assistance #<?= $req['id'] ?></h1>
...
<div class="container" style="max-width:760px;">
    <div class="card">
        <div class="card__header">
            <h2 class="card__title"><?= sanitize($req['assistance_type']) ?> Assistance</h2>
            <?= statusBadge($req['status']) ?>
        </div>
        <div class="card__body">
            <div class="grid-2 mb-3">
                <div><span class="form-label">Assistance Type</span><p><?= sanitize($req['assistance_type']) ?></p></div>
                <div>
                    <span class="form-label">Amount Requested</span>
                    <p style="font-size:22px; font-weight:700; color:var(--accent); font-family:var(--font-heading);">₱<?= number_format($req['amount_requested'],2) ?></p>
                </div>
                ...
                <div style="grid-column:1/-1"><span class="form-label">Description</span><p style="white-space:pre-wrap;"><?= sanitize($req['description']) ?></p></div>
            ...
            <div class="card" style="background:var(--success-bg); border-color:#a7dfc5; margin-bottom:16px;">
                <div class="card__body" style="padding:16px 20px; color:var(--success); font-size:14px;">
                    ✅ <strong>Your application has been approved.</strong> Please visit the Barangay Hall to complete processing. Bring a valid ID.
                </div>
            </div>
            ...
            <div class="card" style="background:var(--danger-bg); border-color:#f5c6c6; margin-bottom:16px;">
                <div class="card__body" style="padding:16px 20px; color:var(--danger); font-size:14px;">
                    ❌ <strong>This application was denied.</strong> Please refer to the remarks above or visit the Barangay Hall for clarification.
                </div>
            </div>
            <?php else: ?>
            <div class="warning-box">⌛ Your application is pending review by the barangay.</div>
            <?php endif; ?>
